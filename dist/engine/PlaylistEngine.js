...
class PlaylistEngine {
    constructor(playlistId, playlistName) {
        this.playlist = new dataStructures_1.DoublyLinkedList();
    ...
    sortPlaylist(criteria, algorithm = 'merge') {
        try {
            ...
            const compareFn = dataStructures_1.SortingUtils.createSongComparator(criteria);
            let sortedSongs;
            if (algorithm === 'merge') {
                sortedSongs = dataStructures_1.MergeSort.sort(songs, compareFn);
            }
            else {
                sortedSongs = dataStructures_1.QuickSort.sort(songs, compareFn);
            }
     ...
     */
    sortPlaylistMultiCriteria(criteriaList) {
        try {
            const songs = this.playlist.toArray();
            const compareFn = dataStructures_1.SortingUtils.createMultiCriteriaComparator(criteriaList);
            const sortedSongs = dataStructures_1.MergeSort.sort(songs, compareFn);
            // Rebuild the playlist with sorted songs
            this.playlist.clear();
            for (const song of sortedSongs) {
                this.playlist.append(song);
...
exports.PlaylistEngine = PlaylistEngine;
//# sourceMappingURL=PlaylistEngine.js.map