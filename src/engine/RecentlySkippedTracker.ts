 ...
 */
export class RecentlySkippedTracker {
  private skippedQueue: Queue<SkippedSong>;
  private maxSkipHistorySize: number;
  private skipCountMap: Map<string, number>; // songId -> skip count

  constructor(maxSkipHistorySize: number = 10) {
    this.maxSkipHistorySize = maxSkipHistorySize;
    this.skippedQueue = new Queue<SkippedSong>(maxSkipHistorySize);
    this.skipCountMap = new Map<string, number>();
  }
  ...
  isSkippedWithinTimeWindow(songId: string, windowMinutes: number): boolean {
    try {
      ...
      return skipHistory.some(skippedSong => 
        skippedSong.song.id === songId && skippedSong.skippedAt >= cutoffTime
      );
    } catch (error) {
  ...
  getMostSkippedSongs(limit: number = 5): Array<{ song: Song; skipCount: number }> {
    try {
      const skipCounts: Array<{ song: Song; skipCount: number }> = [];
  ...
  getSkipStats(): {
    ...
    try {
      ...
      let oldestSkipTime: Date | null = null;
      let newestSkipTime: Date | null = null;

      if (skipHistory.length > 0) {
        const sortedByTime = [...skipHistory].sort((a, b) => 
          a.skippedAt.getTime() - b.skippedAt.getTime()
        );
  ...
  updateMaxHistorySize(newSize: number): void {
    try {
      ...
      if (this.skippedQueue.size() > newSize) {
        ...
        this.skippedQueue = new Queue<SkippedSong>(newSize);
