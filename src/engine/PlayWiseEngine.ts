...
import {
  PlaylistEngine,
  PlaybackHistoryEngine,
  SongRatingEngine,
  InstantSongLookupEngine,
  AutoReplayEngine,
  RecentlySkippedTracker,
  SystemDashboardEngine
} from './index';
...
export class PlayWiseEngine {
  ...
  private dashboardEngine: SystemDashboardEngine;

  // State management
  private currentPlayingIndex: number = -1;
  private isPlaying: boolean = false;
  private isAutoReplayMode: boolean = false;

  constructor(
    playlistId: string = 'default-playlist',
    playlistName: string = 'My Playlist',
    autoReplayConfig?: AutoReplayConfig
  ) {
    // Initialize all engines
    this.playlistEngine = new PlaylistEngine(playlistId, playlistName);
    this.playbackHistoryEngine = new PlaybackHistoryEngine(50); // Max 50 history entries
    this.songRatingEngine = new SongRatingEngine();
    this.lookupEngine = new InstantSongLookupEngine(200); // Initial capacity for 200 songs
    this.autoReplayEngine = new AutoReplayEngine(autoReplayConfig);
    this.skippedTracker = new RecentlySkippedTracker(10); // Track last 10 skipped songs
    
    // Initialize dashboard engine with all sub-engines
    this.dashboardEngine = new SystemDashboardEngine(
      this.playlistEngine,
      this.playbackHistoryEngine,
      this.songRatingEngine,
      this.lookupEngine,
      this.autoReplayEngine,
      this.skippedTracker
    );
  }
   ...
   */
  addSong(song: Song, addToPlaylist: boolean = true): boolean {
    try {
